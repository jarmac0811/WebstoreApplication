...
public List <Product> getProductsByPrice(Map<String,Integer> Params){
	List <Product> productsByPrice=new ArrayList<Product>();
	System.out.println(Params.get("low"));
	Params.get("low");
//	Integer highprice=Params.get("high");
	for(Product product: listOfProducts) 
	if((product.getUnitPrice()>=(Params.get("low")))&&(product.getUnitPrice()<=(Params.get("high"))))
